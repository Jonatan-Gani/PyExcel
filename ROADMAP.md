...
- [x] **SAFE-1** for the UDF: dispatched through `ExcelAsyncUtil.Run`, so the calc thread never blocks. First call returns `#N/A` immediately; the cell auto-refreshes when the kernel returns. Worker-side cancellation (kernel acting on CANCEL frames) is still pending — Excel-DNA cancels its background task on formula change, but the kernel completes the run regardless.
