...
**Current position:** Phase 0 ✅ · Phase 1 ✅ · Phase 2 ✅ · Phase 4 mostly done (everything except items that hinge on Phase 3's COM-bound pieces) · Phase 3 foundation landed (`StateService`, ribbon wiring, `ScriptDirectoryWatcher`). What's left to fully close Phase 3 + Phase 4 is the COM-coupled glue — see [`docs/phase3-and-4-completion.md`](docs/phase3-and-4-completion.md). Working branch: `NET_Migration`.
...
- [ ] Persist per-workbook state as a `CustomXMLPart` on the workbook. **Not in this slice** — needs Excel COM; see `docs/phase3-and-4-completion.md`.
- [ ] `AppEventSink` — `WorkbookOpen` / `Activate` / `SheetActivate` → update state + invalidate ribbon. **Not in this slice** — needs Excel COM event sink; see `docs/phase3-and-4-completion.md`.
...
- [x] **SAFE-1** for the UDF: dispatched through `ExcelAsyncUtil.Run`, so the calc thread never blocks. First call returns `#N/A` immediately; the cell auto-refreshes when the kernel returns.
- [x] Kernel-side cooperative CANCEL — supervisor dispatches `RUN_REQUEST` on a worker thread and pumps inbound frames during the run, so `CANCEL` sets a flag the user's `transform()` can read via `pyexcel.kernel.is_cancelled()`. `PING` continues to answer for liveness. The kernel replies `ERROR / Cancelled` whether the user code noticed and aborted or completed naturally. **Not yet wired through the UDF**: Excel-DNA's task cancellation isn't routed to `KernelClient.Cancel` — that bridge is a UDF follow-up.
...
- [~] Non-blocking progress UI driven by `PROGRESS` frames, with a working **Cancel**. **Kernel half done:** `pyexcel.kernel.report_progress(percent, message)` emits `PROGRESS` frames the supervisor streams to the host (`KernelClient.ProgressReceived` already wired). The WinForms dialog that renders them is the remaining (Windows-only) piece, with Phase 8.
