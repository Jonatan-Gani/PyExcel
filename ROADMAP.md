...
- [~] CSV/TSV import via a real **RFC-4180** parser — embedded newlines/commas/quotes, UTF-8 + BOM detection, correct delimiter for `.tsv`. *(Pure-logic parser `PyExcel.Excel.CsvParser` shipped — string + `Stream` overloads, all three line endings, blank-line semantics matching Python's `csv.reader`, permissive bare-quote handling matching Excel's import. The COM-bound `ImportService` that drives a `Range.Value2` write from the parsed rows is the net48-only follow-up.)*
- [ ] Excel-format import (XLSX/XLSM/XLSB/ODS) via COM; sheet picker that lists the **source** workbook's sheets.
- [~] Export ranges to CSV/Excel with correct quoting and explicit encoding. *(Pure-logic writer `PyExcel.Excel.CsvWriter` shipped — minimal RFC-4180 quoting, configurable line terminator, optional UTF-8 BOM, `Stream` overload. The COM-bound `ExportService` that reads a `Range.Value2` and feeds rows into the writer is the net48-only follow-up.)*
